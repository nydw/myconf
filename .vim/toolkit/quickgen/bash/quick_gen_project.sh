...
gen_filenamelist () 
{
    if test "$support_filenamelist" = "true"; then
        ...
        if test "${dir_filter}" != ""; then
            ...
            find -L ${force_posix_regex_1} . -maxdepth 1 ${force_posix_regex_2} -regex ".*\.("${file_filter}")" > "./${vimfiles_path}/_filenamelist_cwd"
            find -L ${force_posix_regex_1} ${dir_filter} ${force_posix_regex_2} -regex ".*\.("${file_filter}")" >> "./${vimfiles_path}/_filenamelist_cwd"
        else
            find -L ${force_posix_regex_1} . ${force_posix_regex_2} -regex ".*\.("${file_filter}")" > "./${vimfiles_path}/_filenamelist_cwd"
        fi
...
gen_cscope ()
{
    if test "$support_cscope" = "true"; then
        ...
        if [ -f "./cscope.out.in" ]; then
            echo "  |- move cscope.out.in to ./${vimfiles_path}/cscope.out.in"
            mv -f "cscope.out.in" "./${vimfiles_path}/cscope.out.in"
        fi
        if [ -f "./cscope.out.po" ]; then
            echo "  |- move cscope.out.po to ./${vimfiles_path}/cscope.out.po"
            mv -f "cscope.out.po" "./${vimfiles_path}/cscope.out.po"
        fi
